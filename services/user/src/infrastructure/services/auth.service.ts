...
export default class AuthService implements IAuthService {
  private static readonly VALIDATE_AUTH_TOKEN_PATH = '/v1/auth/validate/token';
  ...
  public async validateAuthTokensOrThrowAsync(
    ...
    const { data, error } =
      await this.authHttpClient.postAsync<IAuthTokensValidityResponse>({
        body: this.authMapper.serializeTokenValidityParams(dto),
    ...
    if (typeof data === 'string') {
      this.logger.error(
        `Failed to parse auth tokens validity response: ${data}`,
      );
